...
match = next((v for v in versions if v.get("name") == "12.1.0"), None)
if not match:
    match = next((v for v in versions if v.get("name", "").startswith("12.1.")), None)
if not match:
    match = next((v for v in versions if v.get("name", "").startswith("12.")), None)
if not match:
